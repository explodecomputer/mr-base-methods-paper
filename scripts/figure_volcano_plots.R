...
geom_point(data=subset(fres, sig), aes(colour=category, size=fdr < 0.05)) +
# facet_grid(. ~ exposure, scale="free") +
geom_label_repel(data=subset(fres, sig), aes(label=outcome, colour=category), segment.colour="black", point.padding = unit(0.7, "lines"), box.padding = unit(0.2, "lines"), segment.size=0.5, force=20, max.iter=3e3, alpha=0.9, size=3) +
# geom_text_repel(data=subset(fres, sig), aes(label=outcome, colour=category)) +
theme_bw() + 
theme(panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(),
